...
param(
    ...
    [switch]$AllowCreateDecorationDraft
)
...
if (-not $ConfirmWriteLocalDb) {
    throw 'This smoke writes merchant decoration draft, synthetic micro page/form/submission, and synthetic product records. Re-run with -ConfirmWriteLocalDb to continue.'
}

$repoRoot = Split-Path -Parent $PSScriptRoot
$envPath = Join-Path $repoRoot 'studio-workbench/.env.local'
$scriptPath = Join-Path $PSScriptRoot 'studio-workbench-merchant-card-write-smoke.mjs'
$stamp = Get-Date -Format 'yyyyMMdd-HHmmss'

if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $OutputDir = Join-Path $repoRoot "docs/evidence/studio-merchant-card-write-smoke-$stamp"
}
...
$previousConfirm = $env:STUDIO_SMOKE_CONFIRM_MERCHANT_WRITE
$previousAllowCreateDecorationDraft = $env:STUDIO_SMOKE_ALLOW_CREATE_DECORATION_DRAFT
...
try {
    ...
    $env:STUDIO_SMOKE_CONFIRM_MERCHANT_WRITE = '1'
    $env:STUDIO_SMOKE_ALLOW_CREATE_DECORATION_DRAFT = if ($AllowCreateDecorationDraft) { '1' } else { '0' }

    node $scriptPath
    if ($LASTEXITCODE -ne 0) {
        throw "studio workbench merchant/card write smoke failed with exit code $LASTEXITCODE"
    }
} finally {
    ...
    $env:STUDIO_SMOKE_CONFIRM_MERCHANT_WRITE = $previousConfirm
    $env:STUDIO_SMOKE_ALLOW_CREATE_DECORATION_DRAFT = $previousAllowCreateDecorationDraft
}
