...
const displayTenant = computed(() => {
  const current = tenantList.value.find((item) => item.tenantId === companyName.value);
  return current?.companyName || (userStore.tenantId ? `租户 ${userStore.tenantId}` : '影约云默认租户');
});
