...
export const normalizeDashboardExportDate = (date: string) => date.replace(/-/g, '')

export const buildDashboardExportRangeDays = (beginDate: string, endDate: string) => {
  if (!beginDate || !endDate) return Number.POSITIVE_INFINITY
  const beginTime = Date.parse(`${beginDate}T00:00:00`)
...
export const isDashboardExportInvalidRange = (
  beginDate: string,
  endDate: string,
  rangeDays: number,
) =>
...
export const buildDashboardExportFallbackName = (beginDate: string, endDate: string) =>
