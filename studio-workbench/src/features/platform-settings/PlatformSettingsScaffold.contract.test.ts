...
describe('platform settings scaffold contract', () => {
  ...
  it('mounts phase1 read-only panels for platform facade domains', () => {
    ...
    expect(phase1PanelSource).toContain("$emit('action'")
    ...
    expect(taskCenterSource).toContain('data-testid="platform-task-detail-drawer"')
    expect(taskCenterSource).toContain('backendApi.getPlatformAsyncTaskDetail')
    expect(taskCenterSource).toContain('downloadPlatformAsyncTaskByUrl')
  })
})
