...
describe('studio backend api contract', () => {
  ...
  it('exposes finance reconciliation and async export on the shared reports facade', () => {
    ...
    expect(backendContractSource).toContain('/yy/reportFinanceReconciliation/export/tasks/${encodeURIComponent(taskId)}/download')
    ...
    expect(backendTypesNormalized).toContain('fileName: string')
    expect(backendTypesNormalized).toContain('errorMessage: string')
  })
  ...
  it('aggregates account, finance, platform and tool owner slices on the shared facade', () => {
    ...
    expect(backendPlatformApiSource).toContain('/yy/platform-settings/async-tasks/${encodeURIComponent(taskType)}')
    ...
    expect(backendTypesNormalized).toContain('export type PlatformAsyncTaskRunDto = {')
    expect(backendTypesNormalized).toContain('export type PlatformAsyncTaskDetailDto = {')
